...
TEST_F(PostprocessingUtilsEdgeCaseTest, CreatePredictedObjects_MorePredictionsThanHistory)
{
  ...
  std::vector<float> prediction(OUTPUT_SHAPE[0] * OUTPUT_SHAPE[1] * OUTPUT_SHAPE[2] * OUTPUT_SHAPE[3], 1.0f);
...
TEST_F(PostprocessingUtilsEdgeCaseTest, GetTrajectoryFromPredictionMatrix_ZeroMovement)
{
  ...
  auto trajectory = postprocess::get_trajectory_from_prediction_matrix(
    prediction_matrix, transform, stamp);
...
TEST_F(PostprocessingUtilsEdgeCaseTest, CreateMultipleTrajectories_BoundaryIndices)
{
  ...
  auto empty_trajectories = postprocess::create_multiple_trajectories(
    prediction, stamp, transform, batch_size, 0);
